...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    prompt_segment magenta white "%{$fg_bold[white]%(!.%{%F{white}%}.)%}%m@$USER%{$fg_no_bold[white]%}"
  else
    prompt_segment 229 132 "%{$fg_bold[132]%(!.%{%F{132}%}.)%}%m@$USER%{$fg_no_bold[132]%}"
  fi
}
...
prompt_dir() {
  prompt_segment  black 006 "%{$fg_bold[006]%}%~ :%{$fg_no_bold[006]%}"
}
...
prompt_virtualenv() {
  ...
  if [[ -n $virtualenv_path && -n $VIRTUAL_ENV_DISABLE_PROMPT ]]; then
    prompt_segment black 072 "(`basename $virtualenv_path`)"
  fi
}

prompt_time() {
  prompt_segment black 177 "%{$fg_bold[177]%}%D{%Y-%m-%d %H:%M:%S}%{$fg_no_bold[177]%}"
}
